...
private void CreateChunk(BinaryReader reader) {
        ...
        switch (chunkId) {
            ...
            case MVxPaletteNoteChunk.CHUNK_ID:
                ...
                break;
            case MVxMetaChunk.CHUNK_ID:
                _ = new MVxMetaChunk(reader);
                break;

            default:
                Debug.LogWarning($"Encountered unsupported chunk type \"{chunkId}\" while importing vox model. Skipping.");
    ...
    private class MVxMetaChunk : MVxChunk {
        public const string CHUNK_ID = "META";

        public MVxMetaChunk(BinaryReader reader) : base(reader) {
            if (ChunkSize > 0) {
